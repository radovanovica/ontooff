...
const SECTIONS: HelpSection[] = [
  {
    ...
    title: 'Dashboard',
    path: '/owner',
    tagline: 'Your starting point - see everything at a glance',
    intro:
    ...
    tips: [
      'Pending Bookings is your most important daily metric. Check it every morning.',
  ...
  },
  {
    ...
    title: 'My Places',
    path: '/owner/places',
    tagline: 'The top level of your content - a campsite, resort, or activity venue',
    intro:
      'A Place is your property or venue - for example "Green Valley Camping" or "Adriatic Adventure Park". One account can manage multiple places. Everything else (activity types, locations, spots, pricing, bookings, events) lives inside a place.',
        ...
        steps: [
          {
            ...
            detail: 'The multi-step form wizard opens. You will go through the steps one by one - you can always go back to a previous step.',
          },
          {
  ...
  },
  {
    ...
    path: '/owner/places/[id] - Settings tab',
    ...
    subSections: [
      {
        ...
        steps: [
          { title: 'Name & Slug', detail: 'The name is shown publicly. The slug is the URL identifier - change it only if necessary, as it changes the public link and may break saved bookmarks.' },
          { title: 'Description', detail: 'Write a detailed, welcoming description. This text appears on the public listing and affects search results. Aim for at least 100-200 words.' },
      ...
      },
      {
        title: 'Images & Media',
        steps: [
          { title: 'Profile Photo', detail: 'Your logo or venue icon. Shown as a circular avatar on the listing. Use a square image for best results (min 400x400 px).' },
          { title: 'Cover Photo', detail: 'Wide banner photo shown at the top of your public page. Use a landscape image (min 1200x400 px).' },
          { title: 'Floor Plan / Map Image', detail: 'Upload a plan of your grounds (PNG or JPG). This image is used in the map editor when you create Locations and place them visually on the map. Typical size: 900x600 or larger.' },
        ],
      },
      {
    ...
    tips: [
      ...
      'Upload the floor plan image before creating locations - you will use it to pin each location on the map.',
    ],
  },
  {
    id: 'activity-types',
    ...
    path: '/owner/places/[id] - Activity Types tab',
    tagline: 'Define what kind of activities your place offers',
    intro:
      'An Activity Type is a category of service - for example "Tent Camping", "Glamping Cabin", "Kayaking", or "Archery". You define them here first, then assign one or more activity types to each Location. Activity Types also carry the Pricing Rules.',
    ...
    subSections: [
      {
        ...
        steps: [
          ...
          { title: 'Pick an Icon', detail: 'Enter an emoji (e.g. tent, kayak, arrow) as the icon. It appears on the listing cards and booking forms.' },
  ...
  },
  {
    ...
    title: 'Pricing Rules',
    path: '/owner/places/[id] - Activity Types tab - price tag icon',
    tagline: 'Set how much guests pay - per person, per day, or a flat daily fee',
    intro:
    ...
    subSections: [
      {
        ...
        steps: [
          ...
          },
          {
            ...
            detail:
              'PER_PERSON_PER_DAY - total is calculated as (price x people x days). Use this for campsite pitches or any activity billed per guest per night.\n\nDAILY_FEE - a flat amount charged per day regardless of group size. Use this for facility rentals (e.g. a kayak, a meeting room).',
          },
          {
            title: 'Add Pricing Tiers',
            detail:
              'Each tier represents one type of guest. Start with ADULT. Add more tiers as needed:\n\nADULT - standard adult price\nCHILD - discounted price for children (you set the label, e.g. "Child 3-12 yrs")\nINFANT - typically free or very low price\nCUSTOM - use for any other category (e.g. "Student", "Senior", "Pet")\n\nFor each tier, enter the price in EUR.',
          },
          {
            title: 'Set the Tier Label',
            detail: 'The label is shown to guests in the booking form. Be descriptive: "Adult (13+)", "Child (3-12)", "Infant (under 3 - free)".',
          },
          {
            title: 'Select Payment Method',
            detail:
              'BOTH - guest can pay online or on arrival (recommended).\nONLINE - guest must pay online before the booking is confirmed.\nON_SITE - guest pays on arrival only.',
          },
          {
            title: 'Requires Payment toggle',
            detail:
              'ON - the booking system collects or registers a payment.\nOFF - the activity is free or handled entirely outside the system.',
          },
          {
        ...
        steps: [
          {
            ...
            detail: 'Rule: Adult EUR 15, Child EUR 8. Guest books for 3 nights with 2 adults and 1 child.\nTotal = (2 x 15 x 3) + (1 x 8 x 3) = 90 + 24 = EUR 114.',
          },
          {
            title: 'DAILY_FEE example',
            detail: 'Rule: EUR 45/day. Guest rents a kayak for 2 days.\nTotal = 45 x 2 = EUR 90. Group size does not affect the price.',
          },
          {
    ...
    warnings: [
      ...
      'Setting a price of 0 makes that tier free - use with care.',
    ],
  },
  {
    id: 'locations',
    ...
    path: '/owner/places/[id] - Locations tab',
    tagline: 'Named activity areas within your place - a zone, field, or facility',
    intro:
    ...
    subSections: [
      {
        ...
        steps: [
          { title: 'Click "Add Location"', detail: 'You are taken to the New Location form page.' },
          {
            ...
            detail: 'Give the location a clear name guests will see (e.g. "Zone A - Tent Pitches", "Main Kayak Dock"). The description appears on the public listing.',
          },
          {
          ...
          },
          {
            ...
            detail: 'Select which Activity Types are available at this location. Each activity type has a "Requires Spot" toggle:\n\nON - guests must choose an individual spot (pitch number, cabin, etc.) when booking.\nOFF - the booking is for the location as a whole without assigning a specific spot.',
          },
          {
            title: 'Pin on Floor Plan Map',
            detail: 'If your place has a floor plan image uploaded, an interactive SVG map is shown. Click "Pick on map" then click the exact position of this location on the plan. A circle pin is placed - this helps guests understand where the location is.',
          },
      ...
      {
        title: 'Location Detail - Gallery & GPS',
        steps: [
          {
            title: 'Gallery tab',
          ...
          },
          {
            ...
            detail: 'Enter the latitude and longitude for outdoor locations. Use the map picker to drop a pin on an interactive OpenStreetMap - the coordinates fill automatically. A "Get Directions" link appears for guests.',
          },
          {
    ...
    tips: [
      ...
      'Upload at least 3-5 good photos per location. Listings with photos convert significantly better.',
  ...
  },
  {
    ...
    title: 'Spots',
    path: '/owner/places/[id]/locations/[id] - Spots tab',
    tagline: 'Individual numbered units within a location - a pitch, a cabin, a seat',
    intro:
      'A Spot is the smallest bookable unit inside a location. When "Requires Spot" is ON for an activity type, guests must choose one specific spot when booking. Spots have their own capacity, minimum/maximum stay, status, and amenities. Examples: Pitch 1-30, Cabin A through F, Kayak K01-K10.',
    features: [
    ...
    subSections: [
      {
        ...
        steps: [
          { title: 'Go to location detail - Spots tab', detail: 'Open a location from the Locations tab on your place page, then click the Spots tab.' },
          { title: 'Click "Add Spot"', detail: 'A creation dialog opens.' },
          { title: 'Name', detail: 'The spot display name, e.g. "Pitch 01", "Cabin A", "Kayak K-01". Keep it short and consistent.' },
          ...
          },
          {
            title: 'Status',
            detail:
              'AVAILABLE - open for bookings (default).\nOCCUPIED - currently in use, blocked from new bookings.\nMAINTENANCE - spot is being serviced, blocked from bookings.\nDISABLED - permanently hidden from guests.',
          },
          {
            title: 'Amenities',
        ...
        steps: [
          {
            ...
            detail: 'An SVG overlay of the location map image appears. Draw a rectangle or circle to mark the physical position of this spot. This creates an interactive visual map guests can use to pick their preferred spot.',
          },
        ],
      },
    ],
    tips: [
      'Number spots consistently: Pitch 01, Pitch 02, ... so managing them is easy.',
    ...
    warnings: [
      'Setting a spot to DISABLED prevents it from appearing in booking forms - use MAINTENANCE for temporary blocks instead.',
  ...
  },
  {
    ...
    title: 'Events',
    path: '/owner/places/[id] - Events tab',
    ...
    steps: [
      ...
      { title: 'Name & Description', detail: 'Give the event a name (e.g. "Yoga Sunrise Retreat - July 12") and a detailed description of what guests will experience.' },
  ...
  },
  {
    ...
    title: 'Bookings',
    path: '/owner/bookings  or  /owner/places/[id] - Bookings tab',
    tagline: 'View, approve, and manage all reservations',
    intro:
    ...
    steps: [
      {
        ...
        detail: 'Navigate to Bookings in the left menu for a full list across all your places, or go to Place - Bookings tab for a place-specific view.',
      },
      {
        title: 'Search & filter',
    ...
    tips: [
      'Try to respond to PENDING bookings within 24 hours - guests appreciate fast confirmation.',
      'The payment breakdown shows the exact calculation: price per tier x quantity x days so you can verify totals easily.',
  ...
  },
  {
    ...
    icon: <StarOutlined />,
    title: 'Reviews',
    path: '/owner/places/[id] - Reviews tab',
    ...
    steps: [
      ...
      { title: 'Read guest feedback', detail: 'Each review shows the guest name, star rating (1-5), date, and written comment.' },
  ...
  },
  {
    ...
    icon: <CodeOutlined />,
    title: 'Embed Widget',
    path: '/owner/places/[id] - Locations tab - embed icon  or  Embed Tokens tab',
    tagline: 'Put the booking form directly on your own website',
    intro:
      'The Embed Widget lets you embed a fully functional booking form for a specific location into any website - your own site, a travel blog, or a partner page. Guests can book without ever visiting ontooff.app.',
    ...
    steps: [
      ...
      },
      {
        ...
        detail: 'Click the iframe copy button to get a ready-to-paste HTML snippet. Paste it into your website HTML to show the booking form inline.',
      },
      {
        title: 'Monitor usage',
...
function Callout({ type, children }: { type: 'tip' | 'warning'; children: React.ReactNode }) {
  const map = {
    tip: {
      bg: '#e8f5e9',
      border: '#2e7d32',
      icon: <LightbulbOutlined sx={{ fontSize: 16, color: '#2e7d32', flexShrink: 0 }} />,
    },
    warning: {
      bg: '#fff3e0',
      border: '#e65100',
      icon: <WarningAmberOutlined sx={{ fontSize: 16, color: '#e65100', flexShrink: 0 }} />,
    },
  };
  const c = map[type];
  return (
      ...
      {parts.map((part, i) => (
        <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 0.25 }}>
          <Chip
            ...
            size="small"
            variant="outlined"
            sx={{ fontFamily: 'monospace', fontSize: '0.7rem', height: 20, borderRadius: 1 }}
          />
          {i < parts.length - 1 && (
...
export const metadata = { title: 'Place Owner Help Guide - ontooff' };
    ...
    <Box sx={{ bgcolor: 'grey.50', minHeight: '100vh' }}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.8)', maxWidth: 560 }}>
          Everything you need to know to set up, manage and grow your venue on ontooff - from
          creating your first place to pricing, spots, and bookings.
        </Typography>
      ...
      >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Box key={section.id} id={section.id} sx={{ mb: 5 }}>
              ...
              >
                ...
                >
                  ...
                  <Typography
                    variant="body2"
                    sx={{ lineHeight: 1.7, color: 'text.secondary', maxWidth: 680 }}
                  >
                    {section.intro}
                  </Typography>
                  {section.features && (
                ...
                {section.subSections?.map((sub, si) => (
                  <Box key={si}>
                    ...
                    >
                      {sub.icon && (
                        <Box sx={{ color: section.color, display: 'flex', fontSize: 18 }}>
                ...
                {((section.tips?.length ?? 0) > 0 || (section.warnings?.length ?? 0) > 0) && (
                  ...
                  >
                    {section.tips?.map((tip, i) => (
                      <Callout key={`t${i}`} type="tip">
                        {tip}
                      </Callout>
                    ))}
                    {section.warnings?.map((w, i) => (
                      <Callout key={`w${i}`} type="warning">
                        {w}
                      </Callout>
                    ))}
                  </Box>
          ...
          >
            ontooff Place Owner Guide - For internal use only
          </Typography>
        </Box>
      </Box>
    </Box>
